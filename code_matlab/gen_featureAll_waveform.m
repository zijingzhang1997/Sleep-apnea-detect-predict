Path='C:\Users\zz587-admin\Documents\sleep center\CNN_algorithm\Output_old\';
%FolderName='CNNFeat_pred_v4'; 

FolderName='Feat_detect_v4';% detection folder 
FolderName='Feat_pred_v5';% pred folder 
Path=[Path,FolderName,'\'];

dataPath=[Path,'segments\'];
SavePath=[Path,'featureAll\'];
SaveName=[FolderName,'_DataAll'];
...
for i = 1:30
    ...
    load(filePathName);
    labelNCS_seg=labelNCS;
    labelPSG2_seg=labelPSG2;
    labelPSG_seg=labelPSG;
    
    featureNCS_seg=NCS_data;
    featurePSG_seg=PSG_data;
    featurePSG2_seg=PSG2_data;
...
trueUniq=sum(NCSuniqEpoch)/length(caseNumPSG);
